...
subtitle: 'Tamoxifen Response in ER+ Breast Cancer'
author: '\textbf{AUTHORS} \vspace{0.3cm} \newline \begin{tabular}{ll} 1. WALTER ODUR & 2025\slash HD07\slash 26017U \\ 2. HAGGAI ELISHA KIPUYO & 2025\slash HD07\slash 30926T \end{tabular}'
date: "`r Sys.Date()`"
...
```{r missingness-heatmap, fig.height=4, fig.cap="Missing value pattern across all 2,981 proteins and 56 samples. Blue = detected, grey = missing. Proteins are ordered by total detection (most detected at top). The large grey block at the bottom is the MNAR fingerprint, proteins chronically below the MS detection limit."}
...
```{r missingness-heatmap-zoom, fig.height=5.5, fig.cap="Zoomed heatmap: 300 proteins with the most variable detection pattern. Row clustering groups proteins with similar missingness profiles. Blocks aligned with OR or PD columns indicate group-structured missingness."}
...
```{r qc-total-intensity, fig.height=4, fig.cap="Total log2 LFQ intensity per sample. The dashed line marks 2 standard deviations below the mean. Any sample below this line should be investigated as a potential technical failure."}
...
```{r qc-pca-batch, fig.height=4, fig.cap="PCA of all 56 samples on the unfiltered log2 LFQ matrix. Left panel coloured by clinical group; right panel coloured by sample index as a run-order proxy. If the right panel shows stronger clustering than the left, a batch effect is present."}
...
```{r plot-filter-effect, fig.height=4.5, fig.cap="Log2 LFQ intensity distributions per sample before filtering (top) and after filtering (bottom). Lower whiskers lift slightly after filtering as chronically low-abundance proteins are removed. Overall shape and inter-sample alignment are preserved."}
...
```{r imputation-diagnostic, fig.height=5, fig.cap="Density plots for 6 representative samples before (blue) and after (red) Impseq imputation. Impseq broadens the left tail rather than creating a sharp separated shoulder (characteristic of MinProb). The main abundance peak must remain unchanged in position and height after imputation."}
...
```{r pca-post-imputation, fig.height=5, fig.cap="PCA of all 56 samples after filtering and imputation. Separation between OR (blue) and PD (red) groups indicates a systematic protein expression difference. Dashed ellipses show 95% confidence regions per group."}
...
```{r correlation-heatmap, fig.height=5.5, fig.cap="Pairwise Pearson correlation heatmap across all 56 samples after imputation. Within-group samples should show higher mutual correlation (darker blue) than cross-group pairs."}
...
```{r sva-sensitivity, fig.height=4}
...
```{r volcano-plot, fig.height=4, fig.cap="Volcano plot of differential expression results (PD vs OR). Red = significantly upregulated in PD; blue = upregulated in OR; grey = not significant. Dashed lines mark FDR = 0.05 and log2FC = +/-1 thresholds."}
...
```{r heatmap-top-proteins, fig.height=4.5, fig.cap="Heatmap of top significant differentially expressed proteins (z-scored log2 LFQ intensity). Columns are split by clinical group. Red = high relative expression; blue = low."}
...
```{r boxplots-top8, fig.height=5.5, fig.cap="Expression boxplots for the 8 most significant differentially expressed proteins. Each point is one patient sample. Significance brackets from Wilcoxon test."}
...
```{r go-bp, fig.height=7, fig.cap="GO Biological Process enrichment. Dot size = gene count; colour = adjusted p-value. Only terms significant at FDR < 0.05 are shown."}
...
```{r kegg, fig.height=7, fig.cap="KEGG pathway enrichment of significant differentially expressed proteins."}
...
```{r reactome, fig.height=7, fig.cap="Reactome pathway enrichment of significant differentially expressed proteins."}
...
```{r enrichment-map, fig.height=4, fig.cap="Enrichment map of GO Biological Process terms. Nodes are enriched terms; edges connect terms sharing significant genes. Clusters represent overarching biological themes."}
...
```{r gsea-analysis, fig.height=10, fig.cap="Gene Set Enrichment Analysis (GSEA) of GO Biological Process terms using the full ranked protein list (ranked by limma t-statistic). Unlike ORA, GSEA does not depend on an arbitrary significance cutoff and can detect coordinated changes across gene sets."}
...
```{r gsea-enrichment-plot, fig.height=4, fig.width=9, fig.cap="GSEA enrichment plot for the top-ranked GO Biological Process term. The running enrichment score (green line) shows the cumulative deviation from a uniform distribution as the algorithm walks down the ranked gene list. A sharp peak indicates a concentrated enrichment signal."}
...
```{r ppi-network, fig.height=5.5, fig.cap="STRING protein-protein interaction network of significant differentially expressed proteins. Edges represent interactions with STRING confidence score >= 0.4."}
...
```{r coexpression-network, fig.height=6, fig.cap="Co-expression network of significant and near-significant proteins (FDR < 0.10, |log2FC| >= 0.585). Edges connect proteins with Pearson r >= 0.7 across all 56 samples. Node colour indicates fold-change direction. Singletons (no edges) are excluded."}
...
```{r replication-plot, fig.height=5, fig.cap="Replication scatter plot. Each point is a protein tested in both datasets. Discovery-significant proteins in red; labels mark those also significant in validation. Proteins in the upper-right and lower-left quadrants replicate (same direction in both datasets)."}
